...
export const NewbornView: React.FC<NewbornViewProps> = ({ onOpenBooking }) => {
  const page = SITE_PAGES.newborn;
          ...
          <div className="lg:col-span-7 space-y-6">
            ...
            <div className="bg-white p-5 sm:p-6 rounded-2xl border-2 border-[#D8C2B8] shadow-md space-y-3 font-body text-sm text-[#1C121B]">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 font-semibold text-xs sm:text-sm">
                <p className="flex items-center gap-2"><CheckCircle size={18} className="text-[#2E4323]" weight="fill" /> Full access to hand-knit wraps & headbands</p>
                <p className="flex items-center gap-2"><CheckCircle size={18} className="text-[#2E4323]" weight="fill" /> Eucalyptus wreaths & wooden props</p>
                <p className="flex items-center gap-2"><CheckCircle size={18} className="text-[#2E4323]" weight="fill" /> 2-3 hours unhurried studio time</p>
                <p className="flex items-center gap-2"><CheckCircle size={18} className="text-[#2E4323]" weight="fill" /> Parent & sibling poses welcome</p>
                ...
                <p className="flex items-center gap-2"><CheckCircle size={18} className="text-[#2E4323]" weight="fill" /> Heated & sanitized studio space</p>
              </div>
            </div>

            <div className="pt-2 flex flex-col sm:flex-row items-center gap-4 font-body">
              <button
                onClick={() => onOpenBooking('newborn')}
          ...
          <div className="lg:col-span-5 relative pt-4">
            <div className="relative bg-white p-3.5 pt-4 pb-7 rounded-none shadow-[0_12px_36px_rgba(33,20,32,0.14)] border border-neutral-300 transform -rotate-1 hover:rotate-0 transition-transform duration-500">
              ...
              <p className="font-display text-center text-base text-[#1C121B] font-bold mt-3">
                Northfield Studio Newborn Portrait
              </p>
            </div>
          </div>
        </div>
